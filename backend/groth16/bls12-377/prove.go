...
// Accelerator offloads the heavy operations of Prove: the multi-scalar
// multiplications against the proving key and the computation of the quotient
// polynomial H. Implementations are attached to a ProvingKey with
// SetAccelerator; when none is set, Prove uses gnark-crypto on the CPU.
//
// The bases passed to MultiExpG1 and MultiExpG2 are always slices of the
// proving key (pk.G1.A, pk.G1.B, pk.G1.K, pk.G1.Z, pk.G2.B,
// pk.CommitmentKeys[i].Basis and pk.CommitmentKeys[i].BasisExpSigma), so an
// implementation may cache device-side copies keyed by the backing array.
// Scalars are in Montgomery form. Implementations must be safe for concurrent
// use: Prove issues several MSMs from different goroutines.
type Accelerator interface {
	// MultiExpG1 returns Σ scalars[i]·bases[i].
	MultiExpG1(bases []curve.G1Affine, scalars []fr.Element) (curve.G1Affine, error)
	// MultiExpG2 returns Σ scalars[i]·bases[i].
	MultiExpG2(bases []curve.G2Affine, scalars []fr.Element) (curve.G2Affine, error)
	// ComputeH returns the coefficients of h = (a·b − c)/(Xⁿ−1) where a, b, c are
	// the evaluations of the R1CS polynomials on domain (len(a) <= n, zero padded)
	// and n is domain.Cardinality. The result has length n; only the first n−1
	// entries are used.
	ComputeH(a, b, c []fr.Element, domain *fft.Domain) ([]fr.Element, error)
}
...
func (pk *ProvingKey) multiExpG1(res *curve.G1Jac, bases []curve.G1Affine, scalars []fr.Element, nbTasks int) error {
	if pk.accelerator == nil {
		_, err := res.MultiExp(bases, scalars, ecc.MultiExpConfig{NbTasks: nbTasks})
		return err
	}
	aff, err := pk.accelerator.MultiExpG1(bases, scalars)
	if err != nil {
		return err
	}
	res.FromAffine(&aff)
...
func (pk *ProvingKey) multiExpG2(res *curve.G2Jac, bases []curve.G2Affine, scalars []fr.Element, nbTasks int) error {
	if pk.accelerator == nil {
		_, err := res.MultiExp(bases, scalars, ecc.MultiExpConfig{NbTasks: nbTasks})
		return err
	}
	aff, err := pk.accelerator.MultiExpG2(bases, scalars)
	if err != nil {
		return err
	}
	res.FromAffine(&aff)
...
// pedersenCommit computes the BSB22 commitment of values with the i-th commitment key.
func (pk *ProvingKey) pedersenCommit(i int, values []fr.Element) (curve.G1Affine, error) {
	if pk.accelerator == nil {
		return pk.CommitmentKeys[i].Commit(values)
	}
	if len(values) != len(pk.CommitmentKeys[i].Basis) {
		return curve.G1Affine{}, errors.New("must have as many values as basis elements")
	}
	return pk.accelerator.MultiExpG1(pk.CommitmentKeys[i].Basis, values)
}

// pedersenProveKnowledge computes the BSB22 proof of knowledge of values with the i-th commitment key.
func (pk *ProvingKey) pedersenProveKnowledge(i int, values []fr.Element) (curve.G1Affine, error) {
	if pk.accelerator == nil {
		return pk.CommitmentKeys[i].ProveKnowledge(values)
	}
	if len(values) != len(pk.CommitmentKeys[i].BasisExpSigma) {
		return curve.G1Affine{}, errors.New("must have as many values as basis elements")
	}
	return pk.accelerator.MultiExpG1(pk.CommitmentKeys[i].BasisExpSigma, values)
}

func (pk *ProvingKey) computeH(a, b, c []fr.Element) ([]fr.Element, error) {
	if pk.accelerator == nil {
		return computeH(a, b, c, &pk.Domain), nil
	}
	return pk.accelerator.ComputeH(a, b, c, &pk.Domain)
}
...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, fullWitness witness.Witness, opts ...backend.ProverOption) (*Proof, error) {
	...
	acceleration := "none"
	if pk.accelerator != nil {
		acceleration = fmt.Sprintf("%T", pk.accelerator)
	}
	log := logger.Logger().With().Str("curve", r1cs.CurveID().String()).Str("acceleration", acceleration).Int("nbConstraints", r1cs.GetNbConstraints()).Str("backend", "groth16").Logger()
	...
	solverOpts = append(solverOpts, solver.OverrideHint(bsb22ID, func(_ *big.Int, in []*big.Int, out []*big.Int) error {
		...
		var err error
		if proof.Commitments[i], err = pk.pedersenCommit(i, privateCommittedValues[i]); err != nil {
			return err
		}
	...
	for i := range pk.CommitmentKeys {
		var err error
		if poks[i], err = pk.pedersenProveKnowledge(i, privateCommittedValues[i]); err != nil {
			return nil, err
		}
	}
	...
	go func() {
		h, errH = pk.computeH(solution.A, solution.B, solution.C)
		solution.A = nil
		solution.B = nil
		solution.C = nil
		chHDone <- struct{}{}
	...
	computeBS1 := func() {
		<-chWireValuesB
		if err := pk.multiExpG1(&bs1, pk.G1.B, wireValuesB, n/2); err != nil {
			chBs1Done <- err
			close(chBs1Done)
			return
		}
	...
	computeAR1 := func() {
		<-chWireValuesA
		if err := pk.multiExpG1(&ar, pk.G1.A, wireValuesA, n/2); err != nil {
			chArDone <- err
			close(chArDone)
			return
		}
	...
	computeKRS := func() {
		...
		go func() {
			err := pk.multiExpG1(&krs2, pk.G1.Z, h[:sizeH], n/2)
			chKrs2Done <- err
		}()
		...
		if err := pk.multiExpG1(&krs, pk.G1.K, _wireValues, n/2); err != nil {
			chKrsDone <- err
			return
		}
	...
	computeBS2 := func() error {
		...
		<-chWireValuesB
		if err := pk.multiExpG2(&Bs, pk.G2.B, wireValuesB, nbTasks); err != nil {
			return err
		}
